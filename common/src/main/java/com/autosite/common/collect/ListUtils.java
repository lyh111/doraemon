 ...
 */
package com.autosite.common.collect;
...
import com.autosite.common.callback.MethodCallback;
...
//	}
    ...
    /**
     * 本地列表排序,前提：要排序的值为int数字类型
     *
     * @param list    需要排序的列表
     * @param orderBy 排序的键值（如：id desc）
     * @author lys
     */
    public static <T> List<T> listOrderByInteger(List<T> list, String orderBy) {
        if (list != null && StringUtils.isNotBlank(orderBy)) {
            final String[] ss = orderBy.trim().split(" ");
            if (ss != null && ss.length == 2) {
                Collections.sort(list, new Comparator<T>() {
                    ...
                    public int compare(T o1, T o2) {
                        Integer s1 = 0, s2 = 0;
                        if (o1 instanceof Map) {
                            s1 = ObjectUtils.toInteger(((Map) o1).get(ss[0]));
                            s2 = ObjectUtils.toInteger(((Map) o2).get(ss[0]));
                        } else {
                            s1 = ObjectUtils.toInteger(ReflectUtils.invokeGetter(o1, ss[0]));
                            s2 = ObjectUtils.toInteger(ReflectUtils.invokeGetter(o2, ss[0]));
                        }
                        if ("asc".equals(ss[1])) {
                            return s1.compareTo(s2);
                        } else {
